...
const BilgiToplumu: React.FC = () => {
  ...
  const breadcrumb = (
    <>
      ...
      <span className="text-gray-400 font-bold">/</span>
      <span className="text-white font-bold">{lang === 'tr' ? 'Bilgi Toplumu Hizmetleri' : 'Information Society Services'}</span>
    </>
  );
  ...
  return (
    <div>
      ...
      <Banner
        title={lang === 'tr' ? 'Bilgi Toplumu Hizmetleri' : 'Information Society Services'}
        description={lang === 'tr' ? 'Şirketimizin bilgi toplumu hizmetleri.' : 'Our company’s information society services.'}
        breadcrumb={breadcrumb}
      />
      <div className="bg-white min-h-[40vh] py-16 flex items-center justify-center">
        <div className="max-w-2xl w-full mx-auto text-center">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">{lang === 'tr' ? 'Bilgi Toplumu Hizmetleri' : 'Information Society Services'}</h2>
          <p className="text-gray-700 text-lg whitespace-pre-line">
            {lang === 'tr'
              ? 'Bu bir örnek Bilgi Toplumu Hizmetleri metnidir. Şirketimizin bilgi toplumu hizmetleri kapsamında sunduğu hizmetler burada açıklanmıştır.'
              : 'This is a sample Information Society Services text. The services provided by our company within the scope of information society services are explained here.'}
          </p>
        </div>
      </div>
