...
const content = {
  tr: {
    title: 'Hizmet 5',
    description: 'Beşinci hizmet açıklaması',
    footerLang: 'tr',
    headerLang: 'tr',
  },
  en: {
...
const WebApplications: React.FC = () => {
  ...
  const handleTabChange = (key: string) => {
    ...
    let base = `/${lang}/services/service5`;
  ...
  const breadcrumb = (
    <div className="mt-1 mb-2 text-sm flex items-center space-x-2">
      ...
      <span className="text-gray-400 font-bold">/</span>
      <Link to={`/${lang}/services/service5`} className="text-gray-400 font-bold">{lang === 'tr' ? 'Hizmet 5' : 'Service 5'}</Link>
      ...
      {selectedTab === 'overview' && (
        <>
          <HeroImageTextSection
            ...
            paragraphs={[
              lang === 'tr'
                ? 'Beşinci hizmet açıklaması.'
                : 'Fifth service description.'
            ]}
          />
